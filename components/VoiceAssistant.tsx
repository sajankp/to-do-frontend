  ...
  const toolDefinitions: FunctionDeclaration[] = [
    ...
    },
    {
      ...
      parameters: {
        type: Type.OBJECT,
        properties: {
          search_title: { type: Type.STRING, description: 'The text to search for in the todo title to identify which task to delete.' }
        },
    ...
    },
    {
      ...
      parameters: {
        type: Type.OBJECT,
        properties: {
          search_title: { type: Type.STRING, description: 'The text to search for to identify the task.' },
    ...
    try {
      if (fc.name === 'get_todos') {
        ...
        if (matches.length === 0) {
          ...
          result = { status: 'error', message: `Multiple tasks found: ${matches.map(t => t.title).join(', ')}. Please be more specific.` };
        } else {
        ...
        if (matches.length === 0) {
          ...
          result = { status: 'error', message: `Multiple tasks found: ${matches.map(t => t.title).join(', ')}.` };
        } else {
    ...
    try {
      ...
      const sessionPromise = ai.live.connect({
        ...
        config: {
          ...
          systemInstruction: 'You are a helpful voice assistant managing a todo list. You can add, delete, update, and list tasks. Identify tasks by fuzzy name matching. If a user wants to delete "milk", look for "Buy Milk". Be concise.',
        },
        callbacks: {
          onopen: () => {
            setIsActive(true);
